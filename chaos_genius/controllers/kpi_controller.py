...
from chaos_genius.settings import MAX_RCA_SLACK_DAYS
...
def _get_end_date_for_rca_kpi(kpi_info: dict, end_date: date = None) -> date:
    ...
    while not _is_data_present_for_end_date(kpi_info, end_date):
        ...
        end_date = end_date - timedelta(days=1)
        count += 1
        if count > MAX_RCA_SLACK_DAYS:
            raise ValueError(f"KPI has no data for the last {MAX_RCA_SLACK_DAYS} days.")
