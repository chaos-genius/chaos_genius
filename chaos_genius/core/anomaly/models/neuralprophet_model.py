...
class NeuralProphetModel(AnomalyModel):
    ...
    def predict(self, df: pd.DataFrame, pred_df: pd.DataFrame = None)  -> pd.DataFrame:
        ...
        if pred_df is None:
            future = self.model.make_future_dataframe(df=df, periods=1, n_historic_predictions=True)
        elif pred_df is not None:
            future = self.model.make_future_dataframe(df=df, periods = 0, n_historic_predictions=True)
