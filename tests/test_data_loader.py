...
def test_data_loader(monkeypatch: MonkeyPatch):
    ...
    output_query = f"""select * from "cloud_cost" where "date" < '{end_date.strftime("%Y-%m-%d")} 00:00:00'"""
    assert output_query == dl._build_query().strip()
    ...
    output_query = f"""select * from "cloud_cost" where "date" >= '{start_date.strftime("%Y-%m-%d")} 00:00:00'"""
    assert output_query == dl._build_query().strip()
    ...
    end_date = end_date + timedelta(days=1)
    output_query = f"""select * from "cloud_cost" where "date" >= '{start_date.strftime("%Y-%m-%d")} 00:00:00' and "date" < '{end_date.strftime("%Y-%m-%d")} 00:00:00'"""
    assert output_query == dl._build_query().strip()

    # no end_date, no start_date, days_before
    with pytest.raises(ValueError, match='If days_before is specified, either start_date or end_date must be specified'):
        dl = data_loader.DataLoader(
            kpi_info,
            days_before=1
        )
    ...
    start_date = date(2019, 1, 1)
    days_before = 30
    with pytest.raises(ValueError, match='end_date, start_date and days_before cannot be specified at the same time'):
        dl = data_loader.DataLoader(
            kpi_info,
            end_date=end_date,
            start_date=start_date,
    ...
    end_date = end_date + timedelta(days=1)
    output_query = f"""select * from "cloud_cost" where "date" >= '{start_date.strftime("%Y-%m-%d")} 05:30:00' and "date" < '{end_date.strftime("%Y-%m-%d")} 05:30:00'"""
    assert output_query == dl._build_query().strip()

    data_source["database_timezone"] = "America/New_York"
    start_date = start_date - timedelta(days=1)
    end_date = end_date - timedelta(days=1)
    output_query = f"""select * from "cloud_cost" where "date" >= '{start_date.strftime("%Y-%m-%d")} 20:00:00' and "date" < '{end_date.strftime("%Y-%m-%d")} 20:00:00'"""
    ...
    end_date = end_date + timedelta(days=1)
    output_query = f"""select * from "cloud_cost" where "date" >= '{start_date.strftime("%Y-%m-%d")} 00:00:00 +00:00' and "date" < '{end_date.strftime("%Y-%m-%d")} 00:00:00 +00:00'"""
    assert output_query == dl._build_query().strip()
    ...
    end_date = end_date + timedelta(days=1)
    output_query = f"""select * from "cloud_cost" where "date" >= '{start_date.strftime("%Y-%m-%d")} 00:00:00 +05:30' and "date" < '{end_date.strftime("%Y-%m-%d")} 00:00:00 +05:30'"""
    assert output_query == dl._build_query().strip()
