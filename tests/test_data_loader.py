...
def test_data_loader(monkeypatch: MonkeyPatch):
    ...
    output_query = f"""select * from cloud_cost where "date" >= '{start_date.strftime("%Y-%m-%d %H:%M:%S")}' and "date" < '{end_date.strftime("%Y-%m-%d")}'"""
    # FIXME: start date should be formatted as %Y-%m-%d instead of %Y-%m-%d %H:%M:%S
    assert output_query == dl._build_query().strip()

    # table, end_date, days_before, count
    output_query = f"""select count (*) from cloud_cost where "date" >= '{start_date.strftime("%Y-%m-%d %H:%M:%S")}' and "date" < '{end_date.strftime("%Y-%m-%d")}'"""
    assert output_query == dl._build_query(count=True).strip()
    ...
    output_query = f"""select * from cloud_cost where "date" >= '{start_date.strftime("%Y-%m-%d %H:%M:%S")}' and "date" < '{end_date.strftime("%Y-%m-%d")}' limit 10"""
    assert output_query == dl._build_query().strip()
    ...
    output_query = r"select \* from \(select \* from cloud_cost\) as \"[a-z]{10}\""\
        + f""" where "date" >= '{start_date.strftime("%Y-%m-%d %H:%M:%S")}' and "date" < '{end_date.strftime("%Y-%m-%d")}'"""
    assert re.match(output_query, dl._build_query().strip())
    ...
    output_query = r"select \* from \(select \* from cloud_cost\) as \"[a-z]{10}\""\
        + f""" where "date" >= '{start_date.strftime("%Y-%m-%d %H:%M:%S")}' and "date" < '{end_date.strftime("%Y-%m-%d")}' limit 10"""
    assert re.match(output_query, dl._build_query().strip())
